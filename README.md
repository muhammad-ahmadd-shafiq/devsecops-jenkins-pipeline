...
| Section | What to capture | Filename |
|---|---|---|
| Pipeline Flow | Jenkins stage view showing all stages green end to end | `screenshots/pipeline-overview.png` |
| Secret Scanning | Gitleaks stage console output | `screenshots/gitleaks-scan.png` |
| SAST | Bandit console output showing the scan summary | `screenshots/bandit-scan.png` |
| SCA / Container Scanning | Trivy scan output with CVE counts by severity | `screenshots/trivy-scan.png` |
| SBOM Generation | Generated `sbom.json` or Syft's summary output | `screenshots/sbom-scan.png` |
| Image Signing | Cosign sign/verify output confirming a valid signature | `screenshots/cosign.png` |
| DAST | OWASP ZAP baseline scan summary | `screenshots/zap-scan.png` |
| GHCR Push | GHCR package page showing the pushed, tagged image | `screenshots/ghcr-package.png` |
| Slack Notifications | Slack channel showing a pass and a fail notification | `screenshots/slack-notifications.png` |
| Jenkins Dashboard | Full pipeline run history | `screenshots/build-history.png` |
